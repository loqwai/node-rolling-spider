...
Drone.prototype.setup = function (callback){
  this.logger('RollingSpider#setup', this.peripheral);
  var services = []
  var characteristics = []

  const finish = ()=>{
    this.logger('RollingSpider#finishDiscover')
    this.services = services
    this.characteristics = characteristics
    this.logger('services', _.map(services, 'uuid'))
    this.logger('characteristics', _.map(characteristics, 'uuid'))
    this.handshake(callback)
  }

  this.peripheral.discoverServices([], (error, s)=>{
    services = services.concat(s)
    const call2 = _.after(services.length, finish)
    services.forEach((service)=>{
      service.discoverCharacteristics([], (error, sChars)=>{
        characteristics = characteristics.concat(sChars)
        call2()
      })
    })
  })
